class Game(object):
      ...
      def is_strike(self, frame_idx):
          return self.frame_score[frame_idx][0] == 10 or self.frame_score[frame_idx][1] == 10
          
      def score(self):
          if self.bowl_times == 12:
            return 300
          ...
          for i in xrange(frame_num - 1):
              if self.frame_score[i] == 10: # strike or spare
                 if self.frame_bowls[i][0] == 10 or self.frame_bowls[i][1] == 10: # strike
                    if self.is_strike(i + 1):
                        self.frame_score[i] += 10
                        if self.is_strike(i + 2):
                            self.frame_score[i] += 10
                        else:
