...
class NumMatrix(object):
    def __init__(self, matrix):
        ...
        self.sums = [[0 for _ in xrange(col_num + 1)] for _ in xrange(row_num + 1)]
        # sum the horizontal elems(elements in row)
        for i in xrange(1, row_num + 1):
            for j in xrange(1, col_num + 1):
                self.sums[i][j] += self.sums[i][j - 1] + matrix[i - 1][j - 1]
        # then sum the vertical elems(elements in column)
        for i in xrange(1, row_num + 1):
            for j in xrange(1, col_num + 1):
    ...
    def sumRegion(self, row1, col1, row2, col2):
        ...
        :rtype: int
        """
        # kind of like like calculating area, draw a matrix and you will know it
        # calculate the total area first, then minus the repeated area
